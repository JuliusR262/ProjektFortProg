...
-- Fall 1: Die beiden Terme sind gleich, also gibt es keine Unstimmigkeit, liefer 'Nothing'
ds term1 term2 = if term1 == term2 then Nothing
...
ds' (Comb cName1 cTerm1) (Comb cName2 cTerm2)
    ...
    -- Sucht rekursiv nach einer Unstimmigkeit innerhalb der Termliste eines Literals.
    where dsAll :: [(Term, Term)] -> Maybe (Term, Term)
          ...
          dsAll ((cterm1, cterm2):xs) = let
                                            ...
                                            -- Falls es keine Unstimmigkeit gibt, suche weiter in der Restliste.
...
-- Ruf die Hilfsfunktion auf mit einem leeren Sigma (leerer Substitution) zum Start.
...
unify' term1 term2 sigma = if (apply sigma term1) == (apply sigma term2) then Just sigma
                            ...
                            case (ds (apply sigma term1) (apply sigma term2) ) of
                                ...
                                -- Fall 2: Falls wir eine Variable haben, erstellen wir eine Substitution,
                                -- die diese Variable zu dem rechten Term substituiert und komponieren sie
                                -- an das momentane Sigma. Danach rufen wir unify' rekursiv mit dem neuen
                                -- Sigma auf.
